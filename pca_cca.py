...
image_pca = PCA(n_components=180, whiten=True)
audio_pca = PCA(n_components=180, whiten=True)
av_cca = CCA(n_components=120)

def pca_cca(images, audio, test=False, fusion_style='concate'):
    # PCA
    if test is False:
...
for i in range(epoches):
    ...
    av_test = data_flow.av_generator(batch_size=int(test_num), train=False)
    for index in range(n_batches):
        data, label = av_train.next()
        images, audio = data
        images = images.reshape((-1, 3*50*50))
        new_img, new_audio, cca_feature = pca_cca(images, audio)

        # param_grid = {'C': [1e3, 5e3, 1e4, 5e4, 1e5],
        #               'gamma': [0.0001, 0.0005, 0.001, 0.005, 0.01, 0.1],}
        # svc = GridSearchCV(SVC(kernel='rbf', class_weight='balanced', max_iter=500), param_grid)
        svc.partial_fit(cca_feature, label, classes=[0, 1, 2, 3, 4])
    ...
    images, audio = data
    images = images.reshape((-1, 3*50*50))
    test_img, test_audio, test_feature = pca_cca(images, audio,test=True)
    pred = svc.predict(test_feature)
    idx = np.where(pred == label)
    print('epoch:{} acc:{}'.format(i, len(idx[0]) / test_num))
    print(confusion_matrix(label, pred, labels=range(5)))
